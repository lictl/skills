...
Bootstrap stays local for human review and commit. Prepare and verify changes in the user's local checkout, then leave them uncommitted. Do not publish GitHub issues, PRs, or comments, or commit, push, or merge bootstrap changes. This also applies to companion skills used during bootstrap. The ongoing workflow governs subsequent tasks; its ADR/spec changes separately require human document review before merge.
...
All agents use the shared repository-owner GitHub account. The orchestrator reviews through PR comments and merges ordinary code-only changes once criteria, findings and required checks are satisfied. A PR creating or editing ADRs or specs additionally requires human review and explicit approval of the actual document content before merge, including editorial edits and mixed PRs. Record the human source and document revision; a shared-account agent review is not human approval. Leave these PRs open with auto-merge disabled while review is pending. Report actual GitHub merge blockers if encountered. Preserve useful state before removing finished worktrees. Put this agreement in `AGENTS.md` or a linked project workflow document; bootstrap itself does not start executing the backlog.
...
- Human chat, issue comments or requests establish intent, not review of generated documents. Agents can investigate and propose architecture and decide routine implementation details within that intent. Keep new ADRs Proposed and specs Draft until explicit human approval of their actual text; record authorship separately from review evidence. Human review is required before merging any ADR/spec edit. Changed reviewed content needs renewed review; unrelated code commits or recording approval evidence without changing that content do not invalidate it. Unchanged document references alone do not trigger the gate.
...
- Continue autonomously through the authorized task, including checks and agent review. Complete a reviewable ADR/spec change before requesting its required human review, and leave it unmerged pending approval. Ordinary code-only work remains autonomous. Involve the human for other actual blockers, unresolved material choices or required access/actions while continuing unaffected work.
